...
This pass upgrades detail playback-readiness strip chrome. `DetailPlaybackReadinessUiState` now owns strip radius, container base color/alpha, border width/alpha, padding, spacing, status-dot sizing, headline/summary alpha, summary tone, and chip spacing so playback readiness presentation follows the same tested model contract as first-play guidance and decision chips.

This pass upgrades detail episode selector chrome. `DetailEpisodeSummaryUiState`, `DetailEpisodeSelectorUiState`, and `DetailEpisodeOptionUiState` now own the episode header spacing, action-pill chrome, chip spacing, selector row spacing, card sizing, radius, border width, padding, text spacing, and text alpha values so the detail page's episode switching module follows the same tested model contract as route readiness and first-play guidance.

This pass upgrades detail route-resolution panel chrome. `DetailRouteResolutionUiState` now owns card radius, border width/alpha, content padding, content/header/text spacing, progress size, icon-box sizing/radius/container alpha, status icon size, title/detail/subtitle alpha and tone, and chip spacing so route-matching diagnostics are rendered from the same tested model contract as route status, readiness, and episode selector modules.

This pass upgrades detail route-source selector chrome. `DetailRouteSourceSelectorUiState`, `DetailRouteSourceStatusPillUiState`, `DetailRouteSourceAutoChoiceUiState`, and `RouteSourceGroupUiState` now own selector spacing, header text alpha/tone, status-pill sizing/padding/alpha, auto-choice card border/padding/icon/text/badge chrome, and manual source-filter sizing, padding, spacing, and alpha values so automatic route choice and manual line correction share a tested model-driven contract.

This pass upgrades detail route-candidate card chrome. `RouteCandidateUiState` now owns detail card radius, border width/alpha, content padding, row spacing, accent rail sizing/radius, source-box sizing/radius, text spacing, source/title/primary/protocol/size alpha and tone, and trailing spacing so manual route cards share the same tested model contract as player route rows and route-source selectors.

This pass upgrades player route-option row chrome. `RouteCandidateUiState` now owns player row border width, accent-rail alpha, title alpha, compact/detailed subtitle alpha, detail-line tone/alpha, protocol alpha, size tone/alpha, and action-label icon/text alpha so fullscreen route switching and manual line correction share the same tested route candidate contract as detail-page route cards.
