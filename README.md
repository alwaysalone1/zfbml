...
- Playback speed now persists in local player preferences instead of resetting to `1.0x` after the player is recreated.
- Persisted playback-speed values are normalized through a tested model helper before being applied to ExoPlayer, clamping stale values to the supported `0.5x` to `2.0x` range.
- The speed selector still drives the existing player speed panel and status chips, but selected speed now survives route, episode, and player lifecycle changes.
- App version labels, request user agents, and README notes are now updated to `0.5.238`.
...
- Danmaku effect settings now expose a tested platform mapping preview for Bilibili, Tencent, iQiyi, Youku, and local danmaku sources.
- `PlayerDanmakuSettingsUiState` now uses the same platform-effect resolver as the Canvas renderer, so the UI explains the actual adaptive or global effect applied at draw time.
- The danmaku settings panel now shows per-platform effect badges next to the selectable effect styles.
- App version labels, request user agents, and README notes are now updated to `0.5.234`.

### v0.5.233

English:

- Danmaku rendering now supports multiple selectable effect styles, including platform-adaptive, classic stroke, cinema glow, high-contrast, and lightweight modes.
- `DanmakuSettings.effectStyle` defaults to platform-adaptive rendering, so Bilibili, Tencent, iQiyi, Youku, and local danmaku can resolve to source-appropriate stroke and shadow effects at draw time.
- The danmaku settings panel now exposes tested effect-style option rows while keeping manual danmaku search and calibration flows intact.
- App version labels, request user agents, and README notes are now updated to `0.5.233`.
...
- Search result headers now expose a visible status badge for idle, searching, matched, filtered-empty, no-hit, partial-failure, and failed states.
- `SearchResultsSectionUiState` now keeps result readiness labels and tones in the tested model contract alongside header and empty-state copy.
- Added model coverage for search result status labels and tones across idle, loading, selected-source, selected-empty, and failure states.
- App version labels, request user agents, and README notes are now updated to `0.5.220`.
...
- Automatic danmaku title search now reserves one capped query slot for catalog-provided non-CJK aliases when long CJK alias lists fill the search cap.
- Plain detail fallback titles no longer displace trusted CJK catalog aliases; only Bangumi/catalog original names and aliases can use the reserved fallback slot.
- Added registry coverage for overflowed danmaku alias lists while preserving existing CJK-only alias ordering.
- App version labels, request user agents, and README notes are now updated to `0.5.210`.
...
- Automatic danmaku title matching now adds bounded base-title variants for season, part, TV, OVA, and bracketed suffixes.
- This helps provider searches recover correct mappings when Bangumi titles include labels such as second season or Part 2 while providers index the base anime title.
- Manual danmaku searches still use the exact user-entered query without automatic variant expansion.
- App version labels, request user agents, and README notes are now updated to `0.5.205`.
...
- Danmaku search and calibration notices are now modeled by `PlayerDanmakuOperationNoticeUiState`.
- The player now reuses tested notice builders for blank search, search loading, search results, calibration loading, and calibration result feedback.
- Added unit coverage for danmaku operation notice messages, tones, and line limits.
- App version labels, request user agents, and README notes are now updated to `0.5.203`.
...
- Low-confidence danmaku candidate rows now expose their own manual-review flag.
- Candidate action copy changes from `Set for this episode` to `Confirm this episode` when automatic loading was intentionally skipped.
- Added unit coverage for normal, manual, and low-confidence candidate action semantics.
- App version labels, request user agents, and README notes are now updated to `0.5.200`.
...
- Danmaku candidate rows now expose match confidence labels and tones in `PlayerDanmakuCandidateUiState`.
- Candidate subtitles and badges distinguish manual confirmation, high-confidence automatic matches, review-needed matches, and low-confidence matches.
- Added unit coverage for high, review-needed, manual, and low-confidence danmaku candidates.
- App version labels, request user agents, and README notes are now updated to `0.5.197`.
...
- Fullscreen seek sliders now use `PlayerSeekBarUiState.sliderFocusEnabled` for TV/keyboard focus eligibility.
- Unknown-duration playback keeps the loading track visible while skipping non-seekable progress UI in focus navigation.
- Added unit coverage for seekable and unknown-duration slider focus states.
- App version labels, request user agents, and README notes are now updated to `0.5.189`.
...
- Compact fullscreen entry now uses `PlayerCompactFullscreenActionUiState.focusEnabled` for TV/keyboard focus eligibility.
- `PlayerTinyIconAction` renders compact fullscreen focus state from the model alongside its existing enabled/disabled chrome.
- Added unit coverage for compact fullscreen-entry focus state.
- App version labels, request user agents, and README notes are now updated to `0.5.184`.
...
- Player bottom actions now use `PlayerActionUiState.actionFocusEnabled` for TV/keyboard focus eligibility.
- Disabled next-route, route, episode, and cache actions stay visible with disabled presentation but are skipped by focus navigation.
- Compact recovery buttons now share the same focus eligibility as the underlying player action model.
- App version labels, request user agents, and README notes are now updated to `0.5.182`.
...
- Detail route candidate cards now use `RouteCandidateUiState` for source-initial and source-name base colors.
- `RouteCandidateRow` renders source identity text from tested model chrome instead of local white constants.
- Added unit coverage for detail route source identity base-color chrome.
- App version labels, request user agents, and README notes are now updated to `0.5.174`.
...
- Player route-source strips now use `PlayerRouteSourceStripUiState` for title base color, route-list title base color, and route-list title alpha.
- Player route-source chips now use `PlayerRouteSourceChipUiState` for container, title, and detail base colors instead of keeping those chrome constants in Compose.
- Added unit coverage for player route-source strip and chip chrome.
- App version labels, request user agents, and README notes are now updated to `0.5.167`.
...
- Detail episode headers now use `DetailEpisodeSummaryUiState` for shell radius, header/text spacing, title alpha, summary tone/alpha, action-pill chrome, and chip spacing.
- Detail episode selectors now use `DetailEpisodeSelectorUiState` and `DetailEpisodeOptionUiState` for row spacing, card sizing, radius, border width, padding, spacing, and text alpha.
- Added unit coverage for detail episode header and selector chrome.
- App version labels, request user agents, and README notes are now updated to `0.5.161`.

### v0.5.160

English:

- Detail playback readiness strips now use `DetailPlaybackReadinessUiState` for shell radius, container color/alpha, border, padding, spacing, status-dot sizing, text alpha, summary tone, and chip spacing.
- `DetailPlaybackReadinessStrip` now renders the playback readiness shell from model chrome instead of keeping strip constants inside Compose.
- Added unit coverage for detail playback-readiness strip chrome.
- App version labels, request user agents, and README notes are now updated to `0.5.160`.
...
- Detail decision chips now use `DetailFirstPlayChipUiState` for height, radius, container and border chrome, spacing, text alpha, and fallback value copy.
- `DetailDecisionChip` now renders playback-readiness and first-play chips from model chrome instead of keeping chip constants inside Compose.
- Added unit coverage for detail decision-chip chrome.
- App version labels, request user agents, and README notes are now updated to `0.5.158`.
...
- Detail route focus chips now use `DetailRouteFocusChipUiState` for min height, radius, container alpha, padding, spacing, label alpha, and value alpha.
- `RouteSourceFocusChip` now renders from model chrome instead of keeping route-diagnostic chip constants inside Compose.
- Added unit coverage for detail route-focus chip chrome.
- App version labels, request user agents, and README notes are now updated to `0.5.155`.
...
- Detail route prefetch cards now use `RoutePrefetchUiState` for card radius, container color/alpha, border, padding, header spacing, icon-box chrome, progress track, and item spacing.
- `DetailRoutePrefetchCard` now renders its full shell from model chrome instead of keeping route-warming card dimensions and opacity constants inside Compose.
- Added unit coverage for route-prefetch card shell chrome.
- App version labels, request user agents, and README notes are now updated to `0.5.150`.

### v0.5.149

English:

- Detail route metric chips now use `DetailRouteMetricUiState` for height, radius, container base color, container alpha, padding, spacing, label tone, label alpha, and value alpha.
- `RouteMetricChip` now renders from model chrome instead of keeping route-diagnostic metric dimensions and opacity constants inside Compose.
- Added unit coverage for detail route-metric chip chrome.
- App version labels, request user agents, and README notes are now updated to `0.5.149`.
...
- Fullscreen control-row layout now uses `PlayerFullscreenControlRowUiState` for vertical spacing, action-row spacing, danmaku input weight, and action-bar weight.
- `PlayerFullscreenControlRow` now renders its outer layout from model chrome instead of keeping control-row spacing and weighting constants inside Compose.
- Added unit coverage for fullscreen control-row layout chrome.
- App version labels, request user agents, and README notes are now updated to `0.5.147`.
...
- Route play action pills now use `RoutePlayActionChromeUiState` for height, radius, container alpha, padding, spacing, icon size, icon alpha, and text alpha.
- `RoutePlayActionLabel` now renders from model chrome instead of keeping route-action pill dimensions and opacity constants inside Compose.
- Added unit coverage for route play action pill chrome alongside the shared route/status badge chrome.
- App version labels, request user agents, and README notes are now updated to `0.5.143`.
...
- Compact player fullscreen entry now uses `PlayerCompactFullscreenActionUiState` for content description, width, height, radius, icon size, selected/enabled state, container/content base colors, and disabled alpha values.
- `PlayerCompactInteractionRow` and `PlayerTinyIconAction` now render the compact fullscreen button from model chrome instead of keeping button dimensions and opacity constants inside Compose.
- Added unit coverage for compact fullscreen-entry chrome alongside progress and danmaku-entry state.
- App version labels, request user agents, and README notes are now updated to `0.5.141`.
...
- Player fullscreen status strips now expose container height, radius, base color, alpha, border width/alpha, padding, spacing, status tone, route text alpha, and tag text alpha through `PlayerFullscreenStatusStripUiState` and `PlayerFullscreenStatusTagUiState`.
- `PlayerFullscreenStatusStrip` and `PlayerStatusTinyText` now render from model chrome instead of keeping bottom-overlay status-strip dimensions and opacity constants inside Compose.
- Added unit coverage for fullscreen status strip chrome and tag presentation alongside fallback route/quality/speed values.
- App version labels, request user agents, and README notes are now updated to `0.5.138`.
...
- Player seek feedback now uses `PlayerSeekFeedbackUiState` for text, placement, anti-obstruction margins, vertical offsets, radius, border, padding, icon visibility, icon size, tones, and text alpha.
- `PlayerSeekFeedbackPill` and its overlay position now render from model state instead of keeping double-tap feedback spacing and chrome constants inside Compose.
- Added unit coverage for center/button seek feedback and compact side-gesture feedback chrome.
- App version labels, request user agents, and README notes are now updated to `0.5.136`.
...
- Detail hero chrome now uses `DetailHeroChromeUiState` for title/summary fallback copy, poster metadata, current-episode chips, episode-count chips, layout dimensions, overlay alpha, and action sizing.
- `DetailHero` now renders hero metadata chips and shell dimensions from the model while continuing to reuse the existing first-play, readiness, and route-entry models.
- Added unit coverage for detail hero copy, missing-title/summary fallback, selected-episode chips, empty-episode chips, poster dimensions, button sizing, and overlay alpha.
- App version labels, request user agents, and README notes are now updated to `0.5.129`.
...
- Category browse insight strips now render from model-provided metric tiles instead of hard-coded Compose cards.
- `CategoryBrowseUiState` now owns metric ordering, semantic tones, tile sizing, padding, and spacing for coverage, rating, heat, and source summaries.
- Added unit coverage for category metric ordering, fallback/empty metric values, semantic tones, and layout dimensions.
- App version labels, request user agents, and README notes are now updated to `0.5.126`.
...
- The Discover schedule surface now has model-driven digest, hero, day-chip, and anime-row presentation states for timetable browsing.
- Schedule cards now render title/subtitle, loading/error tone, metric chips, metadata chips, row sizing, poster sizing, and action copy from UI models instead of Compose-local constants.
- Added unit coverage for schedule digest states, day-chip chrome, hero metadata chips, row metadata fallbacks, and layout dimensions.
- App version labels, request user agents, and README notes are now updated to `0.5.124`.
...
- Search results now deduplicate same-title hits in the all-sources view, keeping the best metadata-rich result while preserving the original group order.
- Source filters still show each provider's raw returned results, so provider-specific debugging and fallback inspection remain unchanged.
- Added unit coverage for normalized-title deduplication and raw source-filter visibility.
- App version labels, request user agents, and README notes are now updated to `0.5.118`.
